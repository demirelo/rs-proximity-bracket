...
**Negative side — near capacity [PROVEN constructions; assessment per field].** Crites–Stewart (eprint 2025/2046, Cor. 1) prove $\varepsilon_{\mathrm{ca}}=1$ in a window reaching down to the list-decoding capacity radius $R_{\mathrm{cap}}$, so $\delta^\ast_C$ is provably strictly below Singleton capacity at every field size. Krachun–Kazanin–Haböck (eprint 2026/782) and Kambiré (arXiv:2604.09724, Thm. 1; fleshing out the Krachun–Kazanin sketch) construct, over smooth subgroups of prime fields $p\equiv1\bmod n$, explicit monomial lines $f=X^{rm}$, $g=X^{(r-1)m}$ with $\ge n^C$ (resp. $2^{\Omega_\rho(1/\eta)}$) bad scalars at $\delta=(1-\rho)-2/s$, $s=\Theta(\log n)$ — a direct proximity-gap and CA failure near capacity, via an additive-combinatorics lemma on sums of roots of unity. At deployed field sizes the gap $2/s$ is a *constant*, not $o(1)$; the per-field instantiation of this ceiling, with its exact threshold arithmetic and status tags, is the subject of the companion ledger.
...
- **[Jo26]** S. Jo, *Interleaving Stability for Mutual Correlated Agreement and Curve Decodability*, IACR eprint **2026/891** (May 2026). Generator-MCA interleaving transfer (exact for $|\Omega|\le q$, hence affine-line MCA interleaving-invariant); curve-decodability transfer to row-wise interleavings.
...
- **[KKH26]** D. Krachun, S. Kazanin, U. Haböck, *Failure of proximity gaps close to capacity*, IACR eprint **2026/782** (Apr 2026). The counterexample by its original authors: an affine line with $2^{\Omega_\rho(1/\eta)}$ $\theta$-close points at $\theta=1-\rho-\eta$, $\eta=\Theta_\rho(1/\log n)$, over smooth prime subgroups, via a new additive-combinatorics lemma on sums of roots of unity; includes the list-decoding lower-bound strengthening.
